...
raw_ds <- tryCatch(yaml::read_yaml("datasets.yaml"), error = function(e) list())
...
ds_items <- if (length(raw_ds)) {
  lapply(seq_along(raw_ds), function(i) normalize_entry(as.character(i), raw_ds[[i]]))
} else list()
...
pub_grp <- split_by_year(sort_entries(published))
wrk_grp <- split_by_year(sort_entries(working))
ds_grp <- split_by_year(sort_entries(ds_items))
...
compose_section <- function(grp, empty_msg) {
  ...
  for (i in seq_along(grp$years)) {
    ...
    if (!is.na(y)) {
      out <- c(out, sprintf('<h1>%s</h1>', as.character(y)))
    }
...
DATASETS_HTML <- compose_section(ds_grp, "No datasets listed.")
```
## Papers
```{r}
#| echo: false
